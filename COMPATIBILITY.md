...
`goe2e` has not published a version tag yet. The first tagged release should be `v0.1.0`; until `v1.0.0`, the project follows the Go module convention for pre-v1 APIs.
...
The exported API consists of the types and functions in `pkg`, including `TestConfig`, request/response modifiers, lifecycle statement factories, and diagnostics types.
...
3. Tag the release with a semantic version such as `v0.1.0`.
